...
export async function GET(request: NextRequest) {
  try {
    ...
    const operationPromise = (async () => {
      ...
      // Create cache key
      const cacheKey = `invoices_${branch}_${startDate}_${endDate}`;
      ...
      const tasksWithPayments = await TaskModel.find(taskQuery)
        .sort({ createdAt: 1 })
        .limit(150) // Increase limit since we'll filter by payment dates
        .lean()
      ...
      const result = {
        ...
        message: "Cached invoice report (limited data)"
      };
    ...
    console.error("Error fetching cached invoice report:", error);
