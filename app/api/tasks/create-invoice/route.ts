// app/api/tasks/create-invoice/route.ts
...
export async function PUT(request: Request) {
  try {
    ...
    const operationPromise = (async () => {
      ...
      // Get request data
      const { taskId, products, totalAmount }: RequestPayload = await request.json();
      ...
      const order = await OrderModel.findOne({ orderId: task.orderId }).lean();
      if (!order) {
      ...
      const invoiceData = {
        ...
        whatsappNumber: order.whatsappNumber || task.whatsappNumber || ""
      };
        ...
        TaskModel.findByIdAndUpdate(
          taskId,
          {
            $set: {
              ...
              products: products, // Update products with any changes made during invoice creation
              totalPrice: totalAmount // Update total price if changed
            }
          },
          { new: true, lean: true }
        )
      ...
      const [updatedTask] = await Promise.all(updatePromises);
