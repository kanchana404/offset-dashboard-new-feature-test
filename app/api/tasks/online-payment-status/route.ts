// app/api/tasks/online-payment-status/route.ts
...
interface OnlinePaymentStatusPayload {
  taskId: string;
...
export async function PUT(request: Request) {
  try {
    ...
    const { taskId, action, notes }: OnlinePaymentStatusPayload = await request.json();
    if (!taskId || !action) {
      return NextResponse.json({ error: "TaskModel ID and action are required" }, { status: 400 });
    }
    if (!['confirmed', 'failed'].includes(action)) {
      return NextResponse.json({ error: "Invalid action. Must be 'confirmed' or 'failed'" }, { status: 400 });
    }
    ...
    const onlinePayment = await OnlinePaymentModel.findOne({ taskId });
    if (!onlinePayment) {
      return NextResponse.json({ error: "Online payment record not found" }, { status: 404 });
    }

    if (action === 'confirmed') {
      // Online payment confirmed → move to Completed
      task.status = "Completed";
      task.onlinePaymentStatus = 'confirmed';
      task.onlinePaymentNotes = notes || 'Online payment confirmed successfully';
      onlinePayment.status = 'confirmed';
      onlinePayment.notes = notes || 'Online payment confirmed successfully';

      // ───> **NO inventory changes here** (already done in /complete when TaskModel first moved to Temporary Completed)
    }
    else if (action === 'failed') {
      // Online payment failed → change status to "Returned"
      task.status = "Returned";
      task.onlinePaymentStatus = 'failed';
      task.onlinePaymentNotes = notes || 'Online payment failed';
      onlinePayment.status = 'failed';
      onlinePayment.notes = notes || 'Online payment failed';

      // Reset payment fields so it's ready for a new payment
      const onlinePaymentAmount = (task.fullPayment || 0) - (order.advancePayment || 0);
      ...
      // Track the failed payment reversal in paymentHistory
      if (!task.paymentHistory) {
        task.paymentHistory = [];
      }
      task.paymentHistory.push({
        method: 'online',
        amount: -onlinePaymentAmount, // negative to indicate reversal
        date: new Date(),
        status: 'failed',
        notes: notes || 'Online payment failed'
      });

      // ───> **Restore inventory when online payment fails** (reverse the reduction done in /complete)
    ...
    const message = action === 'confirmed'
      ? "Online payment confirmed successfully. TaskModel marked as Completed."
      : "Online payment failed. TaskModel moved to Returned; ready for new payment.";
